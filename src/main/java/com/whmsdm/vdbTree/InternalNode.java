    ...
    public long[] createVdbTreeMultiScaleFile(boolean valid, long newFcPositon, FileChannel fc) throws IOException {
        ...
        if (valid) {
            ...
            if (this.level == -1) {
                ...
                for (int i = 0; i < blockModelProperty.getPropertiesName().size(); i++) {
                    fc.write(ByteBuffer.wrap(ParserUtil.float2Byte(blockModelProperty.getSingleBlockPropertyValue(blockModelProperty.getPropertiesName().get(i % 5), this.wHilbertCode))));
                }
            }
        } else {
        ...
        BlockModelProperty blockModelProperty = ProcessUtil.vdbTree.getMsbmProperty().getBlockModelPropertyMap().get(0);
        for (int i = 0; i < blockModelProperty.getPropertiesName().size(); i++) {
            PropertyRange propertyRange = this.getPropertyRangeMap().get(blockModelProperty.getPropertiesName().get(i % 5));
